...
def is_trainer(f):
	...
	def wrap(*args, **kwargs):
		if session['prof'] == "3":
			...
			flash('You are probably not a trainer!!, Are you?', 'danger')
			return redirect(url_for('login'))
	return wrap
...
def deletetrainer():
	choices.clear()
	cur = mysql.connection.cursor()
	q = cur.execute("SELECT username FROM trainers")
	b = cur.fetchall()
	for i in range(q):
	...
	if request.method == 'POST':
		username = form.username.data
		q = cur.execute("SELECT username FROM trainers WHERE username != %s", [username])
		b = cur.fetchall()
		new = b[0]['username']
		cur.execute("UPDATE members SET trainer = %s WHERE trainer = %s", (new, username))
...
@app.route('/trainerDash', methods = ['GET', 'POST'])
@is_logged_in
@is_trainer
def trainerDash():
	choices.clear()
	...
	q = cur.execute("SELECT username FROM members WHERE trainer = %s", [session['username']])
	b = cur.fetchall()
	for i in range(q):
		tup = (b[i]['username'],b[i]['username'])
		choices.append(tup)
	...
	return render_template('trainerDash.html', equips = equips, form = form, members = members_under)
...
def edit_profile(username):
	...
	if username != session['username']:
		...
		if session['prof']==3:
			return redirect(url_for('trainerDash', username = username))
